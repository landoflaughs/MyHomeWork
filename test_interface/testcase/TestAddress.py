...
from test_interface.wework.WeworkAddress import WeworkAddress
...
class TestAddress:
    ...
    @pytest.mark.parametrize("user_id", ["tmp"] * 5)
    def test_delete_member(self, user_id):
        print(user_id)
        r = self.address.create_member(user_id, self.name, self.mobile, self.department)
        r = self.address.delete(user_id)
